<template>
  <div class=" text-black w-full xl:w-3/12 lg:w-2/6 md:w-1/2">
    <div @click="gotoCharsertD(id)" v-show="page=='character'" class=" m-5 relative">
      <img  class="w-full rounded-t-lg bg-red" :src="images" alt="">
      ...
      <div :class="[getStatus]" class="w-full border-t-2 border-black p-4 pt-4 rounded-b-lg flex-col flex">
        <span  class="text-sm	 pt-5 font-extrabold">{{title}}</span>
        <span>{{species}}</span>
      </div>
    </div>
...
<script>
  export default {
    ...
    methods : {
      ...
      gotoCharsertD(event){
         this.$router.push({ name: 'CharacterDetailView', query: { id: event } })
      },
    },
